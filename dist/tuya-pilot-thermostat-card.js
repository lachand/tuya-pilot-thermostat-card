 ...
 * SPDX-License-Identifier: BSD-3-Clause
 */
console.warn("The main 'lit-element' module entrypoint is deprecated. Please update your imports to use the 'lit' package: 'lit' and 'lit/decorators.ts' or import from 'lit-element/lit-element.ts'. See https://lit.dev/msg/deprecated-import-path for more information.");const ht={Standby:"Éteint",Comfort:"Confort",ECO:"Éco",Anti_forst:"Hors-gel",Thermostat:"Températures",Programming:"Programmation"},dt={_mode:"mode_entity",_heating:"heating_entity",_window:"window_entity",_fault:"fault_entity",_power:"power_entity",_child_lock:"child_lock_entity"};let ut=class extends nt{constructor(){super(...arguments),this._config={},this._autoStatus=""}setConfig(t){this._config={...t}}_changed(t,e){this._config={...this._config,[t]:e||void 0},this._fire()}_fire(){this.dispatchEvent(new CustomEvent("config-changed",{detail:{config:this._config},bubbles:!0,composed:!0}))}_autoDetect(){const t=this._config.entity;if(!t||!this.hass?.entities)return void(this._autoStatus="Entité climate non définie ou hass.entities indisponible.");const e=this.hass.entities[t];if(!e?.device_id)return void(this._autoStatus="Aucun appareil lié à cette entité.");const i=e.device_id,s=Object.values(this.hass.entities).filter(e=>e.device_id===i&&e.entity_id!==t),n={};for(const t of s){const e=t.unique_id??"",i=t.entity_id,s=i.split(".")[0];for(const[t,s]of Object.entries(dt))if(e.endsWith(t)){n[s]=i;break}"sensor"!==s||n.elec_entity||!e.includes("elec")&&!e.includes("electricity")||(n.elec_entity=i)}const o=Object.keys(n).length;0!==o?(this._config={...this._config,...n},this._autoStatus=`${o} entité(s) détectée(s) automatiquement.`,this._fire()):this._autoStatus="Aucune entité associée détectée."}_picker(t,e,i){const s=Array.isArray(i)?i:[i];return j`
      <ha-selector
        .hass=${this.hass}
        .label=${e}
        .value=${this._config[t]||""}
        .selector=${{entity:{domain:s}}}
  ...
  `,t([lt({type:Object})],ut.prototype,"hass",void 0),t([lt({type:Object})],ut.prototype,"_config",void 0),t([lt({type:String})],ut.prototype,"_autoStatus",void 0),ut=t([rt("tuya-thermostat-card-editor")],ut);let pt=class extends nt{setConfig(t){if(!t.entity)throw new Error("entity (climate) est requis");this.config=t}static getConfigElement(){return document.createElement("tuya-thermostat-card-editor")}static getStubConfig(){return{entity:"climate.thermostat",mode_entity:"select.thermostat_mode"}}get _climate(){return this.hass?.states[this.config.entity]}get _modeEntity(){return this.config.mode_entity?this.hass?.states[this.config.mode_entity]:null}get _windowEntity(){return this.config.window_entity?this.hass?.states[this.config.window_entity]:null}get _faultEntity(){return this.config.fault_entity?this.hass?.states[this.config.fault_entity]:null}get _heatingEntity(){return this.config.heating_entity?this.hass?.states[this.config.heating_entity]:null}get _powerEntity(){return this.config.power_entity?this.hass?.states[this.config.power_entity]:null}get _childLockEntity(){return this.config.child_lock_entity?this.hass?.states[this.config.child_lock_entity]:null}get _elecEntity(){return this.config.elec_entity?this.hass?.states[this.config.elec_entity]:null}render(){if(!this.hass||!this.config||!this._climate)return j`<ha-card><div style="padding:1em;color:var(--error-color)">Configuration manquante — vérifiez l'entity.</div></ha-card>`;const t=this.hass.language||"en",e=(e,i)=>t.startsWith("fr")?i:e,i=this._climate,s=i.attributes.current_temperature,n=i.attributes.temperature,o="off"!==i.state,r=this._modeEntity,a=r?r.state:null,l=r?r.attributes.options??[]:[],c="on"===this._windowEntity?.state,h="on"===this._faultEntity?.state,d="on"===this._heatingEntity?.state,u=this._powerEntity,p=u?parseFloat(u.state):null,f=u?.attributes.unit_of_measurement??"W",_=this._elecEntity,v=_?_.state:null,m=_?.attributes.unit_of_measurement??"",$=this._childLockEntity,y=$?"on"===$.state:null,g=this.config.name||i.attributes.friendly_name||"Thermostat",b=a?ht[a]??a:null;return j`
      <ha-card>
        ${h?j`
        ...
        <div class="header">
          <div>
            <div class="title">
              ...
              ${y?j`<span class="badge" title="${e("Child lock","Verrou enfant")}">🔒</span>`:""}
            </div>
            <div class="status-row">
              ${o?j`<span class="status-chip${d?" heating":""}">
        ...
        ${null!==p||null!==v?j`
          ...
          <div class="footer">
            ...
            ${null!==v?j`<span class="stat">${e("Consumption","Conso")}: <b>${v} ${m}</b></span>`:""}
          </div>
        `:""}
      </ha-card>
